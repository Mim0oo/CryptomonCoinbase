...
def get_etheur_sell_price():
	etheur_sell_price = float(client.get_sell_price(currency_pair = 'ETH-EUR')['amount'])
	return etheur_sell_price

# ETH/EUR buy price getter
def get_etheur_sell_price():
	etheur_buy_price = float(client.get_buy_price(currency_pair = 'ETH-EUR')['amount'])
	return etheur_buy_price

# ETH/EUR buy price getter
def get_btceur_sell_price():
	btceur_sell_price = float(client.get_sell_price(currency_pair = 'BTC-EUR')['amount'])
	return btceur_sell_price

def get_eth_balance():
	url = 'https://api.etherscan.io/api?module=account&action=balance&address='+ETH_ADDRESS+'&tag=latest&apikey='+ETHERSCAN_API_KEY
	r = requests.get(url)
...
def printit():
	...
	print color_cyan('Account balance: ')+str(get_eth_balance())+color_cyan(' ETH')
	print color_cyan('Account balance: ')+str(get_etheur_balance(etheur_sell_price))+color_cyan(' EUR')
	print "------------------------------------------"
...
def eth_monitor():
	...
	if eth_dropped == 0:
		if etheur_sell_price <= ETH_LOW or etheur_sell_price >= ETH_HIGH:
			eth_dropped = 1
			print color_cyan('ETH reached price: ')+str(etheur_sell_price)+color_cyan(' EUR'),
			print color_cyan(' sending mail... '),
			send_mail("Ethereum Monitor", "ETH sell price: "+str(etheur_sell_price))
...
def loopeth():
	...
	if old_price < etheur_sell_price:
		print clock+color_green(" ETH sell price raised: "+str(etheur_sell_price)+" EUR"),
		print color_cyan('Balance: ')+str(get_etheur_balance(etheur_sell_price))+color_cyan(' EUR')
	elif old_price > etheur_sell_price:
		print clock+color_red(" ETH sell price dropped: "+str(etheur_sell_price)+" EUR"),
		print color_cyan('Balance: ')+str(get_etheur_balance(etheur_sell_price))+color_cyan(' EUR')

	#else:
		# Optional tick for the current price
		#print clock+color_cyan(" ETH sell price: ")+str(etheur_sell_price)+color_cyan(" EUR, ")+str(etheur_sell_price * 1.955)+color_cyan(" BGN")
	eth_monitor()
	old_price = etheur_sell_price
